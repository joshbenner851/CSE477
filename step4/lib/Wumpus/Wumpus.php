...
class Wumpus
{
    ...
    const FELL = 2;     ///< Fell into the pitEmpty.php
    ...
    private function moveHelper($ndx){
        $this->current = $this->getRoom($ndx);
        $sroom = $this->current;
        if($sroom->contains(self::WUMPUS,0)){
            return self::EATEN;
        }
        elseif($sroom->contains(self::PIT,0)){
            return self::FELL;
        }
        elseif($sroom->contains(self::BIRDS,0)){
    ...
    public function smellWumpus() {
        ...
        foreach($neighbors as $neighbor) {
            foreach($neighbor->getNeighbors() as $neighNeighbor) {
                if($neighNeighbor->contains(self::WUMPUS,0)) {
                    return true;
                }
            }
        }
    ...
    public function feelDraft() {
        $sroom = $this->current;
        foreach($sroom->getNeighbors() as $neighbor) {
